...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $supplier_name = isset($_POST['supplier_name'])
        ? trim($_POST['supplier_name'])
        : '';
    $supplier_address = isset($_POST['supplier_address'])
    ...
    if ($error == '') {
        $supplier_name_safe = mysql_real_escape_string($supplier_name, $conn);

        $check_query = "
            SELECT id
            FROM suppliers
            WHERE supplier_name = '$supplier_name_safe'
            LIMIT 1
        ";
    ...
    if ($error == '') {
        $supplier_address_safe = mysql_real_escape_string($supplier_address, $conn);
        ...
        if (!$result) {
            ...
            $supplier_id = mysql_insert_id($conn);
...
<title>Add Purchased-From (Supplier) - Wifonic Hardware</title>
<style>
...
<div class="page">
    ...
    <div class="glass-card">
        <div class="header">
            <div>
                <div class="title">Add Purchased-From (Supplier)</div>
                <div class="subtitle">Add a place devices are purchased from</div>
            </div>
        ...
        <form method="post" action="add_supplier.php">
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input
                        ...
                        id="supplier_name"
                        name="supplier_name"
                        value="<?php echo htmlspecialchars($supplier_name, ENT_QUOTES, 'UTF-8'); ?>"
                        required
                    >
                </div>
                ...
                <div class="form-group full">
                    <label for="supplier_address">
                        Supplier / Purchased From Address
                    </label>
                    <input
                        type="text"
                        id="supplier_address"
                        name="supplier_address"
                        value="<?php echo htmlspecialchars($supplier_address, ENT_QUOTES, 'UTF-8'); ?>"
                    >
                </div>
            </div>
            ...
            <div class="actions">
                ...
                <button type="submit" class="button submit">Save Supplier</button>
            </div>
        </form>
    </div>
    ...
    <div class="glass-card">
        <div class="header">
            <div class="title" style="font-size: 20px;">Existing Suppliers</div>
        </div>
        ...
        <?php } else { ?>
            <table>
                ...
                <?php foreach ($suppliers as $p) { ?>
                    <tr>
                        <td><?php echo htmlspecialchars($p['supplier_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars($p['supplier_address'], ENT_QUOTES, 'UTF-8'); ?></td>
                    </tr>
                <?php } ?>
            </table>
        <?php } ?>
